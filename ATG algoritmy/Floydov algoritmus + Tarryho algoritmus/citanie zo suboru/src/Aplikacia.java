...
public class Aplikacia {
    ...
    private int[][] maticaC;
    private int[][] maticaX;
    private HashMap<Integer, Integer> objaveneVrcholy;

    public Aplikacia() throws FileNotFoundException {

        //otvori pozadovany subor
        this.subor = new File("citanie zo suboru/subory/pr4.hrn"); //staci zmenit pr1 na pr2 a zmenia sa data...
        try {
...
//Zaciatok tarryho algoritmu                                                                                        <<<
    public void tarryhoAlgoritmus(int pociatocnyVrchol) {
        ...
        this.objaveneVrcholy = new HashMap<Integer, Integer>(this.hashMap.size() + 1);
        if (pociatocnyVrchol < 0 || pociatocnyVrchol > this.hashMap.size()) {
        ...
        int[][] maticaTarryX = new int[this.pocetRiadkov][this.pocetRiadkov]; //matica vzdialenosti
        int[][] maticaTarryY = new int[this.pocetRiadkov][this.pocetRiadkov]; //matica podmienok
...
//        }
        ...
        //cyklus pre pridavanie nul po hlavnej diagonale
        for (int i = 1; i < maticaTarryX.length; i++) {
        ...
        //naplni maticu vzdialenostami; matica tarry X
        for (int k = 1; k <= this.pocetRiadkov; k++) {
...
//        //vypisovanie hlavneho pola, to su tie data, co su v prX.hrn, kde X je cislo suboru...
//        for (int i = 1; i < this.pocetRiadkov; i++) {
//             for (int j = 1; j < 4; j++) {
//                 System.out.print(this.pole[i][j] + " ");
//             }
