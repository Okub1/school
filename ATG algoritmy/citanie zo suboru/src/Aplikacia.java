...
public class Aplikacia {
    ...
    private int[][] maticaZaklC;
    private int[][] maticaZaklT;
    private int[][] maticaZaklX;
    private HashMap<Integer, Boolean> mnozinaE_HM;
    private ArrayList<Integer> mnozinaE_AL;

    public Aplikacia() throws FileNotFoundException {

        //otvori pozadovany subor
        this.subor = new File("subory/pr4.hrn"); //staci zmenit pr1 na pr2 a zmenia sa data...

        /*
        * pr1 = priklad na floyda z prednasky
        * pr2 = ten velky subor
        * pr3 = priklad na tarryho z prednasky
        * pr4 = priklad na zakladny z prednasky
        * pr5 = priklad na dijkstrov z prednasky
        */
        try {
...
//        zistiCestu(3,4);
    ...
    public void zistiCestu(int vrcholZ, int vrcholDo) {
        if (vrcholZ <= this.smerniky.size() + 1 || vrcholDo <= this.smerniky.size() + 1) {
...
//Zaciatok zakladneho algoritmu                                                                                     <<<
    public void zakladnyAlgoritmus(int pociatocnyVrchol) {
        int velkostZakl = this.smerniky.size() + 1;
        this.maticaZaklC = new int[velkostZakl][velkostZakl]; // matica vzdialenosti (ceny)
        this.maticaZaklT = new int[velkostZakl][velkostZakl]; // matica horneho odhadu dlzky doteraz najdenej najlepsej cesty
        this.maticaZaklX = new int[velkostZakl][velkostZakl]; // matica predposlednych vrcholov



        //kontrola spravneho pociatocneho vrcholu, ak je vrchol mimo platnych vrcholov, oznami chybu...
        ...
        //prida indexy matic zaklC, zaklT, zaklX
        for (int i = 1; i < velkostZakl; i++) {
            this.maticaZaklC[i][0] = i;
            this.maticaZaklT[i][0] = i;
            this.maticaZaklX[i][0] = i;
            this.maticaZaklC[0][i] = i;
            this.maticaZaklT[0][i] = i;
            this.maticaZaklX[0][i] = i;
        }

        //                      naplnanie matic
        //naplni maticu C hodnotami zo suboru
        for (int k = 1; k <= this.pocetRiadkov; k++) {
            this.maticaZaklC[this.pole[k][1]][this.pole[k][2]] = this.pole[k][3];
        }

        //naplni maticu T
        for (int k = 1; k <= this.pocetRiadkov; k++) {
            if (this.pole[k][3] > 0) {
                this.maticaZaklT[this.pole[k][1]][this.pole[k][2]] = Integer.MAX_VALUE;
            }
        ...
        //vypise maticu zaklC
        System.out.println("\nMatica C");
        for (int i = 0; i < this.maticaZaklC.length; i++) {
            for (int j = 0; j < this.maticaZaklC.length; j++) {
                System.out.print(this.maticaZaklC[i][j] + " ");
            }
            System.out.println();
        }
        ...
        //vypise maticu zaklT
        System.out.println("\nMatica T");
        for (int i = 0; i < this.maticaZaklT.length; i++) {
            for (int j = 0; j < this.maticaZaklT.length; j++) {
                System.out.print(this.maticaZaklT[i][j] + " ");
            }
            System.out.println();
        }
        ...
        //vypise maticu zaklX
        System.out.println("\nMatica X");
        for (int i = 0; i < this.maticaZaklX.length; i++) {
            for (int j = 0; j < this.maticaZaklX.length; j++) {
                System.out.print(this.maticaZaklX[i][j] + " ");
            }
            System.out.println();
        }
    }

    private void najdiSusedaZakladny() {
...
//        }
    ...
    public void tarry(int vrchol) {
        ...
        int sused = this.najdiSusedaTarry(vrchol);
    ...
    private int najdiSusedaTarry(int vrchol) {
...
//Zaciatok Label-Set a Label-Correct

    public void labelSet(int pociatocnyVrchol) {
        int velkostLS = this.smerniky.size() + 1;
        this.mnozinaE_HM = new HashMap<>();
...
//Koniec Label-Set a Label-Correct                                                                                  <<<
}